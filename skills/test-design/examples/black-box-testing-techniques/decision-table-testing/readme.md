# Exemplos de Decision Table Testing

Cada exemplo abaixo mantém a descrição do cenário e acrescenta o **método sob teste** e os **testes Jest** derivados pela técnica (uma coluna da tabela de decisão **não-colapsada** = um caso de teste). Os testes seguem **AAA estrito**: ARRANGE → ACT (uma chamada) → ASSERT (um `expect`). Cada teste é um `it()` separado, com nome que **começa por um verbo no infinitivo** (`rejeitar`, `retornar`, `aprovar`, `validar`…) e em seguida descreve o comportamento validado e a regra exercitada.
